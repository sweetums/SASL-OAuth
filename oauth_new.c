			    ...
			    sasl_out_params_t *oparams)
{
  ...
  */
  ...
  case 200:
    ...
    /* 
    ** If we got an access token then now is the time to 
    ** write it to the disk cache.
    */
    ccache_lock(text);
    ccache_update(text, params);
    ccache_unlock(text, 1);
				 ...
				 sasl_out_params_t *oparams)
{
  ...
  */
  if (!text->token_sent) {
    ...
    if (time(NULL) >= text->access_expires) {
				 ...
				 sasl_out_params_t *oparams)
{
  ...
  if (SASL_OK == result) {
    result = findthis(params, tmpstr, DISCOVERY_HDR_TURI, &(text->refresh_uri));
    if (SASL_OK == result) {
				  ...
				  sasl_out_params_t *oparams)
{
  ...
  ccache_lock(text);
  if (!text->username) {
    result = ccache_fetch(conn_context, params);

    /*
    ** If we still don't have a username we'll be prompting for one
    ** so we won't unlock.  Eventually we'll unlock once the access
    ** token is gotten or the file is closed on mech_dispose.
    */
    if (text->username) {
      ccache_unlock(text, 1);
      text->state = CSTATE_TOKEN;
  ...
  /* XXXXXX I think I need to completely refacotr this. */
				      ...
				      const sasl_utils_t *utils)
{
  ...
  if (text->refresh_uri) utils->free(text->refresh_uri);
